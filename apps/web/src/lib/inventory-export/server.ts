...
export async function createInventoryExport(formData: FormData): Promise<InventoryExportState> {
  ...
  if (error) return { status: "error", message: "Data laporan gagal dibaca." };
  ...
  if (jobError) return { status: "error", message: "Job export gagal dibuat." };
  ...
  try {
    await writeInventoryAudit(supabase, requestedSensitive ? "INVENTORY_EXPORT_SENSITIVE_REQUESTED" : "INVENTORY_EXPORT_REQUESTED", jobId, {
      ...
      row_count: reports.length,
      include_sensitive: requestedSensitive,
      sensitive_reason: requestedSensitive ? sensitiveReason : null,
