...
export async function POST() {
  ...
  for (const job of importJobs ?? []) {
    const { data: rows, error: rowsError } = await supabase
      ...
      .select("id, item_image_storage_path, pickup_evidence_storage_path")
    ...
    if (paths.length > 0) {
      const { error: removeError } = await supabase.storage.from(INVENTORY_IMPORT_BUCKET).remove(paths);
      if (removeError) {
        summary.failedJobs += 1;
        continue;
      }
