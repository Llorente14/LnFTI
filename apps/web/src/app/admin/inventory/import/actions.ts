...
async function markImportRowFailed(supabase: DatabaseClient, rowId: string, safeCode: string) {
  await supabase
...
export async function previewInventoryImportAction(
  ...
  try {
    ...
    return { status: "error", message: error instanceof Error ? error.message : INVALID_FILE };
  }
  ...
  if (uploadError) return { status: "error", message: "Workbook gagal disimpan ke Storage privat." };
...
export async function updateInventoryImportRowAction(formData: FormData) {
  ...
  const normalized = normalizeEditableInventoryRow({
    ...
    itemImageSha256: String(formData.get("itemImageSha256") ?? "") || null,
  });
  ...
  for (const rowId of rowIds) {
    ...
    if (row.item_image_storage_path) {
      ...
      const { error: uploadError } = await supabase.storage.from("report-images").upload(
        ...
        { cacheControl: "3600", contentType: extensionContentType(extension), upsert: false },
      );
