...
### 4.1 Route operasional utama

Gunakan route aktual berikut saat demo, smoke test, dan audit:

| Area | Route | Catatan akses |
| --- | --- | --- |
| Browse publik | `/`, `/reports`, `/reports/[id]` | Tanpa session hanya melihat data dari view publik. |
| Auth | `/register`, `/auth/check-email`, `/auth/confirm`, `/login` | `/auth/confirm` memproses `token_hash` Supabase dan redirect ke `next` yang disanitasi. |
| Student | `/report/new`, `/me/reports`, `/me/claims`, `/me/profile` | Memerlukan session student; report submit harus mencapai `PENDING_REVIEW`. |
| Claim publik | `/reports/[id]?claim=1` | Claim hanya valid untuk laporan `FOUND` yang sudah eligible. |
| Verifier/admin | `/admin`, `/admin/reports`, `/admin/reports/[id]`, `/admin/claims`, `/admin/claims/[id]`, `/admin/handovers` | Dijaga oleh role `verifier` atau `admin`. |
| AI proxy | `/api/ai/analyze-image` | Same-origin browser request; web server meneruskan ke AI service dengan token server-only. |

Jangan menambahkan route fiktif pada runbook. Bila route berubah, update dokumen ini pada PR yang sama dengan perubahan route atau pada tiket dokumentasi berikutnya.
...
2. Buka `/admin`, lalu pilih antrean yang relevan:
   - `/admin/reports` untuk laporan `PENDING_REVIEW` dengan filter `type`, `category`, dan `page`;
   - `/admin/claims` untuk klaim dengan filter `status`, `category`, `overdue`, dan `page`;
   - `/admin/handovers` untuk klaim yang sudah siap serah-terima dan riwayat handover.
...
- `id` job sebagai referensi audit;
- `requested_by` yang mengarah ke profil requester;
...
- pesan error internal yang sudah disanitasi;
- waktu selesai, kedaluwarsa, dibuat, dan diperbarui.
...
## 14. Branch, PR, dan commit policy

LNFTI memakai policy otomatis untuk branch, judul PR, dan pesan commit. Dokumentasi operasional dianggap belum selesai bila policy ini gagal.
...
Tipe yang valid: `feature`, `fix`, `chore`, `docs`, `test`, `refactor`, `spike`, `ci`, dan `build`.

Untuk LNFTI-30, branch valid adalah:

```text
docs/LNFTI-30-operation-guide
```

Judul PR harus memakai format:

```text
[LNFTI-30] Add MVP operation guide
```

Pesan commit harus memakai format:

```text
LNFTI-30 docs(runbook): add MVP operation guide
```

Setiap commit harus satu perubahan logis, dapat direview sendiri, dan tidak memakai pesan `WIP`, `final`, atau `fix again`. Jika branch salah nama, jangan menganggap PR selesai walaupun isi dokumen sudah benar.
...
## 16. Monitoring dan smoke test pascadeploy
...
## 17. Checklist penutupan LNFTI-30
...
- [x] branch, PR title, dan commit policy didokumentasikan;
...
Selama dua item ekspor tersebut belum selesai, PR LNFTI-30 sebaiknya tetap draft atau tiket tetap menunggu dependency LNFTI-34. Jika branch/PR policy gagal, perbaiki branch atau buat replacement PR sebelum meminta review operasional.
...
| 0.2 | 2026-06-17 | Validasi route, admin queue, export_jobs, dan branch/PR policy. |
