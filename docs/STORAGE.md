...
Upload UI is deferred to `LNFTI-16`. Public report image delivery is deferred to `LNFTI-17` and should use controlled server-side delivery or signed URLs. Browser code must not use a service-role key. No remote `supabase db push` was performed for this setup.
