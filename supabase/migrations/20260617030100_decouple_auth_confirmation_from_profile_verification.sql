...
alter column verification_status set default 'UNVERIFIED'::public.profile_verification_status;

update public.profiles
set
  verification_status = case
    when verification_status::text in ('VERIFIED', 'verified') then 'VERIFIED'::public.profile_verification_status
    else 'UNVERIFIED'::public.profile_verification_status
  end,
  verified_at = case
    when verification_status::text in ('VERIFIED', 'verified') then coalesce(verified_at, now())
    else null
  end
where verification_status::text in (
  'PENDING_EMAIL',
  'UNVERIFIED',
  'unverified',
  'pending',
