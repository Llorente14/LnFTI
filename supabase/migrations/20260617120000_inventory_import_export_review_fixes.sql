...
-- Prevent two concurrent jobs from both completing the same normalized inventory row.
...
-- Admins may maintain export jobs owned by other operators; verifiers remain owner-only.
...
-- Audit calls exposed to application code are restricted to known lifecycle actions
-- and to entities the caller owns, except for admins.
...
-- The overloaded import RPC from the hardening migration remains the source of truth.
-- This trigger prevents a caller from attaching a permanent image outside the planned report folder.
...
-- Expiry metadata is handled in SQL, while actual object deletion must use the Storage API.
-- Direct DELETE from storage.objects can leave object bytes orphaned and is intentionally removed.
